...
def get_proj(hist, var, var_range, var_file, pt_range, sig_or_bkg):
    """
    Get the variable and pt from the tree.
    """
    var_val = array('f', [ 0. ])
    t.SetBranchAddress(f'f{var}', var_val)
...
def main():
    ...
    parser.add_argument("bkg_input_file", help="input background tree AnalysisResults_tree.root file")
    ...
    sig_trees = []
    for key in infile_sig.GetListOfKeys():
        if key.GetName(): # is the dataframe directory
            sig_dirs.append(key.Get(tree_name))

    # Prepare canvas and histograms for each var
    ...
    for tree in sig_trees:
        ...
        for i in tree.GetEntries():
            tree.GetEntry(i)
            for var, leaf in var_leaves:
                hists[var].Fill(leaf.GetValue())
